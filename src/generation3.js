Warning: truncated output (original token count: 32049)
...
async function incidentsPage() {
  ...
  root.querySelector("#new-incident")?.addEventListener("cli…2049 tokens truncated…form-wrap" hidden style="margin-top:14px"></div>
...
async function accountsPage() {
  ...
  const currentStaffIds = new Set(g3.directory
    .filter((entry) => ["active", "training", "on_leave"].includes(entry.status))
    .map((entry) => entry.uid || entry.id));
  const formerStaffIds = new Set(g3.directory
    .filter((entry) => entry.status === "former")
    .map((entry) => entry.uid || entry.id));
  ...
  const staffCount = accounts.filter((entry) => currentStaffIds.has(entry.uid || entry.id)).length;
  ...
  const openAccount = async (uid) => {
    ...
    const isStaffAccount = currentStaffIds.has(uid);
  ...
  const renderAccounts = () => {
    ...
    const render = () => {
      ...
      results.innerHTML = filtered.length ? `<div class="registry-list">${filtered.map((account) => {
        ...
        return `<article class="registry-row"><div class="registry-main"><div class="registry-name"><strong>${safe(account.displayName || account.discordUsername || "Cognitus Account")}</strong>${currentStaffIds.has(uid) ? `<span class="registry-staff">STAFF</span>` : formerStaffIds.has(uid) ? `<span class="registry-former-staff">FORMER STAFF</span>` : ""}</div><p>${safe(account.discordUsername || "No Discord username")} · ${safe(account.discordId || "No Discord ID")}</p><small>${safe(account.cognitusId || uid)}${org ? ` · ${safe(org.name)}` : account.organizationId ? ` · Org ${safe(account.organizationId)}` : ""}</small></div><div class="registry-badges">${badge(account.role)} ${badge(account.status)} ${account.identityVerified === true ? `<span class="badge active">Verified</span>` : `<span class="badge">Unverified</span>`}</div><div class="registry-actions"><button class="button button-small ${canManageRegistry && account.role !== 'owner' ? 'button-dark' : ''}" type="button" data-open-account="${safe(uid)}">${canManageRegistry && account.role !== 'owner' ? "Edit" : "View"}</button>${currentStaffIds.has(uid) ? `<a class="button button-small" href="#/staff/${safe(uid)}">Staff</a>` : ""}</div></article>`;
